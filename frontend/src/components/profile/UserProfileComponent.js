...
const UserProfileComponent = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (role.toLowerCase() === 'customer') {
          ...
          profileEndpoint = `/admins/${userId}/profile`; // Adjust if your admin profile endpoint is different
        } else {
  ...
  const getEditProfileRoute = () => {
    return role ? `/${role.toLowerCase()}/profile/update` : '/profile/update'; // Adjust as needed
  };

  const getDashboardRoute = () => {
    return role ? `/${role.toLowerCase()}/dashboard` : '/dashboard'; // Adjust as needed
  };

  const getChangePasswordRoute = () => {
    return role ? `/${role.toLowerCase()}/profile/change-password` : '/profile/change-password'; // Adjust as needed
  };
  ...
  return (
    <div className="container mt-5">
        ...
        <div className="card-body">
          ...
          <div className="row mb-3">
            <div className="col-md-6">
              <strong>First Name:</strong> <span className="ms-2 text-dark">{profile.firstName || profile.user?.firstName || 'N/A'}</span>
            </div>
            <div className="col-md-6">
              <strong>Last Name:</strong> <span className="ms-2 text-dark">{profile.lastName || profile.user?.lastName || 'N/A'}</span>
            </div>
          </div>
          <div className="row mb-3">
            <div className="col-md-6">
              <strong>Email:</strong> <span className="ms-2 text-dark">{profile.email || profile.user?.email || 'N/A'}</span>
            </div>
            <div className="col-md-6">
              <strong>Phone Number:</strong> <span className="ms-2 text-dark">{profile.phoneNumber || profile.user?.phoneNumber || profile.phone_number || 'N/A'}</span>
            </div>
          </div>
          <div className="mb-3">
            <strong>Address:</strong> <span className="ms-2 text-dark">{profile.address || profile.user?.address || 'N/A'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="d-flex gap-2">
        ...
        <button onClick={() => navigate(getDashboardRoute())} className="btn btn-outline-secondary">Back to Dashboard</button>
      </div>
    </div>
  );
};
